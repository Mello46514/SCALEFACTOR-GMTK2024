...
func _physics_process(_delta: float) -> void:
	...
	if $Control2/Button3.is_hovered():
		$Control2/Button3.position.x = lerp($Control2/Button3.position.x,$Node2D.position.x,0.5)
	else:
		$Control2/Button3.position.x = lerp($Control2/Button3.position.x,$Node2D3.position.x,0.5)
